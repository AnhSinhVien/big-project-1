...
#ifndef PWM16BITSETUPFREQ_H
#define PWM16BITSETUPFREQ_H

static unsigned int freq;

void PWM_16Bit_SetupFrequency(int timer, unsigned int frequency){
  ...
  if(timer == 1){
    ...
    TCCR3A |= (1 << COM3A1) | (1 << COM3C1) | (1 << WGM31);    // Clear OCnA, OCnB, OCnC on Compare Match
...
void PWM_16Bit_Frequency(uint16_t* regPin, unsigned long percentPWM){
	if(percentPWM > 255){
  	*regPin = freq;
	}
