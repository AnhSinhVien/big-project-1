...
function printReceipt() {
  // lấy các thẻ html trong id order-details
  const donHangCuaBan = document.getElementById("order-details").innerHTML;
  const tempDivHoaDon = document.createElement("div"); // gán vào thẻ div mới
  tempDivHoaDon.innerHTML = donHangCuaBan;
  // lấy bàn
  const inBan = tempDivHoaDon.querySelector("h3").textContent; // Lấy nội dung của thẻ h3 duy nhất (Bàn)
  // Lấy tất cả các món trong class cacMon và tạo chuỗi HTML từ chúng
  const inCacMon = tempDivHoaDon.querySelectorAll(".cacMon");
  let inCacMonHTML = ""; // Chuỗi chứa nội dung các món cho vào thẻ h4
  inCacMon.forEach((Mon) => {
    inCacMonHTML += `<h4>${Mon.textContent}</h4>`;
  });

  // tổng thành tiền
  const tienBienLai = document.querySelector("p").textContent;
